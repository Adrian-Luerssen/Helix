/**
 * Workspace Manager
 * Creates and manages git workspaces for strands and git worktrees for goals.
...
/**
 * Sanitize a strand name for use as a directory name.
...
/**
 * Build the workspace directory path for a strand.
 * @param {string} baseDir - HELIX_WORKSPACES_DIR
 * @param {string} strandId - Strand ID (e.g. strand_abc123)
 * @param {string} strandName - Human-readable strand name
 * @returns {string}
 */
export function strandWorkspacePath(baseDir, strandId, strandName) {
  const slug = sanitizeDirName(strandName);
  const suffix = strandId.replace(/^strand_/, '').slice(0, 8);
  return join(baseDir, `${slug}-${suffix}`);
}

/**
 * Build the worktree directory path for a goal inside a strand workspace.
 * @param {string} strandWs - Strand workspace root path
...
/**
 * Create a git-initialized workspace directory for a strand.
 ...
 * @param {string} strandName - Strand name (used for slug)
 * @param {string} [repoUrl] - Optional git repo URL to clone
 * @returns {{ ok: boolean, path?: string, existed?: boolean, error?: string }}
 */
export function createStrandWorkspace(baseDir, strandId, strandName, repoUrl) {
  const wsPath = strandWorkspacePath(baseDir, strandId, strandName);
  ...
  try {
    ...
    if (repoUrl) {
      ...
      execSync('git commit --allow-empty -m "Initial commit"', {
        cwd: wsPath,
        stdio: 'pipe',
        env: { ...process.env, GIT_AUTHOR_NAME: 'Helix', GIT_AUTHOR_EMAIL: 'helix@localhost', GIT_COMMITTER_NAME: 'Helix', GIT_COMMITTER_EMAIL: 'helix@localhost' },
      });
    }
...
/**
 * Create a git worktree for a goal inside a strand workspace.
...
/**
 * Remove an entire strand workspace directory.
 *
 * @param {string} strandWs - Strand workspace root path
 * @returns {{ ok: boolean, error?: string }}
 */
 ...
 * Works from either the worktree path or the strand workspace root.
 * No-op if no remote is configured.
 *
 * @param {string} gitDir - Path to run git from (worktree or strand workspace)
...
export function mergeGoalBranch(strandWs, branch) {
  try {
    ...
    // Merge from the main working tree (strand root)
