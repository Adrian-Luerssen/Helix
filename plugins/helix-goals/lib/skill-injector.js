...
function loadSkillFile(type) {
  ...
  try {
    skillCache.pmStrand = existsSync(SKILL_PM_STRAND_PATH)
      ? readFileSync(SKILL_PM_STRAND_PATH, 'utf-8')
      : null;
  } catch {
    skillCache.pmStrand = null;
  }
...
export function clearSkillCache() {
  skillCache = { pm: null, pmStrand: null, pmGoal: null, worker: null, agent: null, loadedAt: null };
}

/**
 * Get PM skill context for injection into agent prompts
 * @param {object} options - Context options
 * @param {string} [options.strandId] - Current strand ID
 * @param {string} [options.strandName] - Current strand name
...
export function getPmSkillContext(options = {}) {
  ...
  if (strandId && strandName) {
    header.push(`- **Project:** ${strandName} (${strandId})`);
  }
...
/**
 * Get strand-level PM skill context for injection into strand PM agent prompts.
 * Instructs the PM to break a project into goals rather than tasks.
 *
 * @param {object} options - Context options
 * @param {string} [options.strandId] - Strand ID
 ...
 * @returns {string|null} Strand PM skill context or null if unavailable
 */
export function getStrandPmSkillContext(options = {}) {
  const skillContent = loadSkillFile('pmStrand') || loadSkillFile('pm');
  ...
  // Append dedicated strand PM skill content (or fallback to generic PM)
  if (skillContent) {
    return header.join('\n') + skillContent;
  }
 ...
 * @param {string} [taskContext.strandId] - Strand ID (if applicable)
 * @param {string} [taskContext.strandName] - Strand name (if applicable)
...
export function getWorkerSkillContext(taskContext = {}) {
  ...
  if (strandName) {
    header.push(`- **Project:** ${strandName}`);
  }
...
export function getSkillAvailability() {
  return {
    ...
    pmStrand: existsSync(SKILL_PM_STRAND_PATH),
