...
export function createClassificationLog(dataDir) {
  ...
  function append(fields) {
    ...
    const entry = {
      ...
      tier: fields.tier ?? 1,
      predictedStrand: fields.predictedStrand ?? null,
  ...
  function recordReclassification(sessionKey, previousStrand, newStrand) {
    const data = load();
    // Find the most recent entry for this session that predicted previousStrand
    const entry = [...data.entries].reverse().find(
      e => e.sessionKey === sessionKey && e.predictedStrand === previousStrand
    );
    if (entry) {
      entry.accepted = false;
      entry.correctedTo = newStrand;
      entry.feedbackMs = Date.now();
      save(data);
      return entry;
    }
    ...
    const synth = {
      ...
      sessionKey,
      tier: 0, // synthetic
      predictedStrand: previousStrand,
      confidence: 0,
      reasoning: 'reclassification',
      latencyMs: 0,
      accepted: false,
      correctedTo: newStrand,
      feedbackMs: Date.now(),
    };
