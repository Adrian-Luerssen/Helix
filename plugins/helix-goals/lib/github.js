 ...
 * Used by strands.create to auto-provision GitHub repos when agent account is configured.
 */
 ...
 * @param {string} strandName - Strand name (used for repo name)
 ...
 */
export async function initGitHubRepo(repoPath, githubConfig, strandName, description) {
  ...
  // Sanitize strand name for repo name
  const repoName = strandName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 100) || 'project';
  ...
  try {
    ...
    try {
      repo = await createRepo(agentToken, repoName, {
        org,
        description: description || `${strandName} — managed by Helix`,
      });
    } catch (err) {
      if (err.status === 422) {
