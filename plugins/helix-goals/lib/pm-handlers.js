...
import { getPmSession, getAgentForRole, getDefaultRoles, getOrCreatePmSessionForGoal, getOrCreatePmSessionForStrand } from './agent-roles.js';
...
export function createPmHandlers(store, options = {}) {
   ...
   * Params: { strandId: string, goalId: string, message: string, pmSession?: string }
   * Response: { response: string, pmSession: string, history: Array }
   */
  handlers['pm.chat'] = async ({ params, respond }) => {
    const { strandId, goalId, message, pmSession: overrideSession } = params || {};

    if (logger) {
      logger.debug(`pm.chat called with: strandId=${strandId}, goalId=${goalId}, messageLen=${message?.length || 0}`);
    }

    if (!strandId) {
      if (logger) logger.warn('pm.chat: missing strandId');
      return respond(false, null, 'strandId is required');
    }
    ...
    try {
      ...
      if (!strand) {
        return respond(false, null, `Strand ${strandId} not found`);
      }
      ...
      if (goal.strandId !== strandId) {
        return respond(false, null, `Goal ${goalId} does not belong to strand ${strandId}`);
      }
      ...
      // Build context-enriched message with SKILL-PM.md + strand/goal info
      ...
      if (wsOps && strand?.workspace?.path) {
        const { snapshot } = buildProjectSnapshot(strand.workspace.path);
      ...
      const contextPrefix = [
        `[SESSION IDENTITY] You are the Goal PM for "${goal.title}" in project "${strand.name}" (strand: ${strandId}, goal: ${goalId}). This is an ISOLATED session — only plan tasks for THIS goal. Do NOT reference or plan for other projects.`,
        '',
      ...
      if (logger) {
        logger.info(`pm.chat: prepared message for ${targetSession}, goal "${goal.title}" in strand "${strand.name}"`);
      }
  ...
  /**
   * pm.getConfig - Get PM configuration for a strand
  ...
  handlers['pm.getConfig'] = ({ params, respond }) => {
    ...
    try {
      ...
      const resolvedPmSession = getPmSession(store, strandId);

      respond(true, {
        pmSession: strand.pmSession || null,  // Strand-specific setting (may be null)
  ...
  /**
   * pm.setConfig - Set PM configuration for a strand
   * Params: { strandId: string, pmSession?: string }
   * Response: { ok: boolean }
   */
  handlers['pm.setConfig'] = ({ params, respond }) => {
    const { strandId, pmSession } = params || {};
    ...
    try {
      ...
      if (pmSession !== undefined) {
        // Allow null to clear strand-specific setting (fall back to global)
        strand.pmSession = pmSession || null;
      }
  ...
  /**
   * pm.getAgent - Get the PM agent ID for a strand
   * Params: { strandId?: string }
  ...
  handlers['pm.getAgent'] = ({ params, respond }) => {
    ...
    try {
      const pmSession = getPmSession(store, strandId);
  ...
  handlers['pm.getHistory'] = ({ params, respond }) => {
    ...
    try {
      ...
      const pmSession = goal.pmSessionKey || getPmSession(store, goal.strandId);
  ...
  /**
   * Get or initialize PM chat history for a strand
   */
  function getStrandPmHistory(strand) {
    if (!Array.isArray(strand.pmChatHistory)) {
      strand.pmChatHistory = [];
    }
    return strand.pmChatHistory;
  }

  /**
   * Add a message to strand PM chat history
   */
  function addToStrandHistory(strand, role, content, maxHistory = DEFAULT_HISTORY_LIMIT) {
    const history = getStrandPmHistory(strand);
    history.push({
      role,
      content,
      timestamp: Date.now(),
  ...
  /**
   * pm.strandChat - Send a message to the strand-level PM
   * Params: { strandId: string, message: string }
   * Response: { enrichedMessage, pmSession, history, strandId }
   */
  handlers['pm.strandChat'] = async ({ params, respond }) => {
    ...
    try {
      ...
      // Save user message to strand history
      const userMessage = message.trim();
      addToStrandHistory(strand, 'user', userMessage);
      store.save(data);

      // Get/create strand PM session (registers in sessionStrandIndex)
      const { pmSessionKey } = getOrCreatePmSessionForStrand(store, strandId);

      // Build context-enriched message with strand PM skill context
      const goals = data.goals.filter(g => g.strandId === strandId);
      ...
      const strandPmSkillContext = getStrandPmSkillContext({
        strandId,
        strandName: strand.name,
      ...
      const contextPrefix = [
        `[SESSION IDENTITY] You are the PM for strand "${strand.name}" (ID: ${strandId}). This is an ISOLATED session — do NOT reference context, goals, or conversations from any other strand or project.`,
        '',
        strandPmSkillContext || null,
        projectSnapshotContext || null,
        '',
        `[Strand PM Context]`,
        `Strand: ${strand.name} (${strandId})`,
        `Active Goals: ${activeGoals.length}`,
      ...
      if (logger) {
        logger.info(`pm.strandChat: prepared message for ${pmSessionKey}, strand "${strand.name}"`);
      }

      const history = getStrandPmHistory(strand).slice(-20);
      ...
      if (logger) {
        logger.error(`pm.strandChat error: ${err.message}`);
      }
      respond(false, null, err.message);
    }
  };
  ...
  /**
   * pm.strandSaveResponse - Save a strand PM assistant response to history
   * Params: { strandId: string, content: string }
   * Response: { ok, hasPlan, strandId }
   */
  handlers['pm.strandSaveResponse'] = ({ params, respond }) => {
    const { strandId, content } = params || {};
    ...
    try {
      ...
      addToStrandHistory(strand, 'assistant', content.trim());
      strand.updatedAtMs = Date.now();
      store.save(data);
      ...
      if (logger) {
        logger.info(`pm.strandSaveResponse: saved response for strand "${strand.name}" (hasPlan: ${hasPlan})`);
      }
  ...
  /**
   * pm.strandGetHistory - Get strand PM chat history
   * Params: { strandId: string, limit?: number }
   * Response: { messages, strandId, strandName, total }
   */
  handlers['pm.strandGetHistory'] = ({ params, respond }) => {
    const { strandId, limit = 50 } = params || {};
  ...
  /**
   * pm.strandCreateGoals - Create goals from a strand PM plan
   * Params: { strandId: string, planContent?: string }
   * If no planContent, uses last assistant message from strand's pmChatHistory
   * Response: { ok, goalsCreated, goals: [{id, title, taskCount}], strandId }
   */
  handlers['pm.strandCreateGoals'] = ({ params, respond }) => {
    const { strandId, planContent } = params || {};
    ...
    try {
      ...
      if (!contentToParse) {
        // Try last assistant message from strand PM history
        const history = getStrandPmHistory(strand);
      ...
      if (!contentToParse) {
        return respond(false, null, 'No plan content provided and no PM response found in strand history');
      }
      ...
      for (const goalData of parsedGoals) {
        ...
        // Store the strand PM's task suggestions as description context for the goal PM.
        const tasks = [];
        ...
        const goal = {
          ...
          autonomyMode: strand.autonomyMode || null,
          worktree: null,
          tasks,
          sessions: [],
          files: [],
        ...
        if (wsOps && strand.workspace?.path) {
          const wtResult = wsOps.createGoalWorktree(strand.workspace.path, goalId, goalData.title);
          if (wtResult.ok) {
            ...
            if (strand.workspace.repoUrl && wsOps.pushGoalBranch) {
              ...
              if (pushResult.pushed || pushResult.ok) {
                ...
                if (logger) logger.info(`pm.strandCreateGoals: pushed branch ${wtResult.branch} to remote for goal ${goalId}`);
              } else {
                goal.pushStatus = 'failed';
                goal.pushError = pushResult.error || 'Push failed';
                if (logger) logger.warn(`pm.strandCreateGoals: failed to push branch ${wtResult.branch}: ${pushResult.error}`);
              }
            }
          } else if (logger) {
            logger.error(`pm.strandCreateGoals: worktree creation failed for goal ${goalId}: ${wtResult.error}`);
          }
        }
      ...
      // Store plan content on strand for reference
      strand.pmPlanContent = contentToParse;
      strand.updatedAtMs = now;
      store.save(data);

      if (logger) {
        logger.info(`pm.strandCreateGoals: created ${createdGoals.length} goals for strand "${strand.name}"`);
      }

      // NOTE: We do NOT auto-send to goal PMs here. The frontend will call
      // pm.strandCascade which saves prompts to pmChatHistory and sends via chat.send.
      ...
      if (logger) {
        logger.error(`pm.strandCreateGoals error: ${err.message}`);
      }
      respond(false, null, err.message);
    }
  };
  ...
  /**
   * pm.strandCascade - Prepare goal-level PM sessions for cascade planning/execution
   * Params: { strandId: string, mode: 'plan' | 'full' }
   ...
   */
  handlers['pm.strandCascade'] = async ({ params, respond }) => {
    const { strandId, mode } = params || {};
    ...
    try {
      ...
      // Find goals in this strand that need planning (no tasks yet)
      const goals = data.goals.filter(g => g.strandId === strandId && g.status !== 'done');
      ...
      if (wsOps && strand?.workspace?.path) {
        try {
          ...
          if (logger) logger.warn(`pm.strandCascade: project snapshot failed: ${e.message}`);
        }
      }

      // Build project goals summary once (sibling goals context)
      const projectGoalsSummary = (() => {
        const lines = [`## Project Goals Summary — "${strand.name}"`];
      ...
      for (const goal of goalsNeedingPlanning) {
        ...
        const userPrompt = `Plan tasks for this goal: "${goal.title}"` +
          (goal.description ? `\n\nDescription:\n${goal.description}` : '') +
          `\n\nThis goal is part of the "${strand.name}" project. Break it into actionable tasks with agent assignments.`;
        ...
        // 4. PM Mode Context (strand name, goal title)
        // 5. User Prompt
        const contextPrefix = [
          `[SESSION IDENTITY] You are the Goal PM for "${goal.title}" in project "${strand.name}" (strand: ${strandId}, goal: ${goal.id}). This is an ISOLATED session — only plan tasks for THIS goal. Do NOT reference or plan for other projects.`,
          '',
          projectSnapshotContext || null,
          '',
          projectGoalsSummary,
      ...
      // Store cascade mode + pending goals on strand for tracking
      strand.cascadeMode = mode;
      strand.cascadePendingGoals = cascadeGoals.map(g => g.goalId);
      strand.updatedAtMs = Date.now();
      store.save(data);

      if (logger) {
        logger.info(`pm.strandCascade: prepared ${cascadeGoals.length} goal PMs for strand "${strand.name}" (mode: ${mode})`);
      }
      ...
      if (gatewayRpcCall) {
        for (const g of cascadeGoals) {
          try {
            ...
            if (logger) logger.info(`pm.strandCascade: backend chat.send OK for ${g.pmSessionKey}`);
          } catch (err) {
            sendResults.push({ goalId: g.goalId, ok: false, error: err.message });
            if (logger) logger.error(`pm.strandCascade: backend chat.send FAILED for ${g.pmSessionKey}: ${err.message}`);
          }
        }
      }
      ...
      if (logger) {
        logger.error(`pm.strandCascade error: ${err.message}`);
      }
      respond(false, null, err.message);
    }
  };
   ...
   * Like pm.strandCascade but for a single goal.
  ...
  handlers['pm.goalCascade'] = async ({ params, respond }) => {
    ...
    try {
      ...
      if (!goal.strandId) {
        return respond(false, null, `Goal ${goalId} has no strandId`);
      }

      const strand = data.strands.find(c => c.id === goal.strandId);
      if (!strand) {
        return respond(false, null, `Strand ${goal.strandId} not found`);
      }
      ...
      // Build shared context — roles, skill, project snapshot (reused from pm.strandCascade)
      const goals = data.goals.filter(g => g.strandId === goal.strandId && g.status !== 'done');
      const allTasks = goals.flatMap(g => g.tasks || []);
      const pendingTasks = allTasks.filter(t => t.status !== 'done');
      ...
      const contextPrefix = [
        `[SESSION IDENTITY] You are the Goal PM for "${goal.title}" in project "${strand.name}" (strand: ${goal.strandId}, goal: ${goal.id}). This is an ISOLATED session — only plan tasks for THIS goal. Do NOT reference or plan for other projects.`,
        '',
        projectSnapshotContext || null,
        '',
        projectGoalsSummary,
