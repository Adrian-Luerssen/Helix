...
/**
 * Deep-merge global + strand service configs. Tokens are NOT included — only
 * service names and non-secret metadata (org, team, model, etc.) are returned.
 */
export function resolveEffectiveServices(data, strandId) {
  const globalServices = data.config?.services || {};
  if (!strandId) return { ...globalServices };
  const strand = data.strands.find(c => c.id === strandId);
  if (!strand) return { ...globalServices };
...
export function createTaskSpawnHandler(store) {
  return function handler({ params, respond }) {
    try {
      ...
      // Build spawned agent context: project summary (if in strand) + goal state + task assignment + worker skill
      ...
      // Resolve workspace path: goal worktree > strand workspace > null
      const workspacePath = goal.worktree?.path || strand?.workspace?.path || null;

      // Resolve effective services (global + strand overrides) for agent context
