/**
 * Autonomy Manager - Handles autonomy modes for tasks and strands
 * 
 * Autonomy Modes:
 ...
 * Resolution chain: task > goal > strand > default('plan')
 * @param {object} task - Task object
 * @param {object} [goal] - Goal object (optional)
 * @param {object} [strand] - Strand object (optional)
 * @returns {string} Effective autonomy mode
 */
export function resolveAutonomyMode(task, goal, strand) {
  ...
  // Fall back to strand-level setting
  if (strand?.autonomyMode && AUTONOMY_MODES.includes(strand.autonomyMode)) {
    return strand.autonomyMode;
  }
...
/**
 * Set default autonomy mode for a strand
 * @param {object} store - Goals store instance
 * @param {string} strandId - Strand ID
 * @param {string} mode - Autonomy mode to set
 * @returns {{ success: boolean, error?: string, strand?: object }}
 */
export function setStrandAutonomy(store, strandId, mode) {
  if (!AUTONOMY_MODES.includes(mode)) {
    return { success: false, error: `Invalid mode. Must be one of: ${AUTONOMY_MODES.join(', ')}` };
  }
  ...
  if (!strand) {
    return { success: false, error: `Strand ${strandId} not found` };
  }
  
  strand.autonomyMode = mode;
  strand.updatedAtMs = Date.now();
  
  store.save(data);
  
  return { success: true, strand };
}
...
export function getTaskAutonomyInfo(store, goalId, taskId) {
  ...
  let strand = null;
  if (goal.strandId) {
    strand = data.strands.find(c => c.id === goal.strandId);
  }

  const mode = resolveAutonomyMode(task, goal, strand);
  ...
  return {
    ...
    strandMode: strand?.autonomyMode || null,
  };
}
...
export function createAutonomyHandlers(store) {
  ...
  handlers['autonomy.setStrand'] = ({ params, respond }) => {
    const { strandId, mode } = params || {};
    
    if (!strandId || !mode) {
      return respond(false, null, 'strandId and mode are required');
    }
    
    const result = setStrandAutonomy(store, strandId, mode);
    
    if (!result.success) {
      return respond(false, null, result.error);
    }
    
    respond(true, { strand: result.strand, mode });
  };
