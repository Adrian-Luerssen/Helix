...
export function createGoalHandlers(store, options = {}) {
  ...
  return {
    ...
    'goals.create': ({ params, respond }) => {
      try {
        const { title, strandId, description, completed, status, priority, deadline, notes, tasks, autonomyMode } = params;
        ...
        // Create worktree if parent strand has a workspace
        if (wsOps && strandId) {
          ...
          if (strand?.workspace?.path) {
            ...
            if (wtResult.ok) {
              ...
              if (strand.workspace.repoUrl) {
                try {
                  const pushResult = pushBranch(strand.workspace.path, wtResult.branch, { setUpstream: true });
                  if (!pushResult.ok && logger) {
                    logger.warn(`helix-goals: goals.create: failed to push branch ${wtResult.branch}: ${pushResult.error}`);
              ...
              logger.error(`helix-goals: worktree creation failed for goal ${goalId}: ${wtResult.error}`);
            }
          }
        }
    ...
    'goals.update': ({ params, respond }) => {
      try {
        ...
        const allowed = ['title', 'description', 'status', 'completed', 'strandId', 'priority', 'deadline', 'notes', 'tasks', 'nextTask', 'dropped', 'droppedAtMs', 'files', 'plan', 'autonomyMode', 'phase', 'dependsOn', 'closedAtMs'];
    ...
    'goals.delete': async ({ params, respond }) => {
      try {
        ...
        if (wsOps && deletedGoal.worktree?.path && deletedGoal.strandId) {
          const strand = data.strands.find(c => c.id === deletedGoal.strandId);
          if (strand?.workspace?.path) {
            const rmResult = wsOps.removeGoalWorktree(strand.workspace.path, deletedGoal.id, deletedGoal.worktree?.branch);
            if (!rmResult.ok && logger) {
              logger.error(`helix-goals: worktree removal failed for goal ${params.id}: ${rmResult.error}`);
            }
          }
        }
    ...
    'goals.setSessionStrand': ({ params, respond }) => {
      try {
        const { sessionKey, strandId } = params;
        if (!sessionKey || !strandId) {
          respond(false, undefined, { message: 'sessionKey and strandId are required' });
          return;
        }
        const data = loadData();
        const strand = data.strands.find(c => c.id === strandId);
        ...
        data.sessionStrandIndex[sessionKey] = strandId;
        saveData(data);
        respond(true, { ok: true, sessionKey, strandId });
      } catch (err) {
        respond(false, undefined, { message: String(err) });
      }
    },

    'goals.getSessionStrand': ({ params, respond }) => {
      try {
        const data = loadData();
        respond(true, { strandId: data.sessionStrandIndex[params.sessionKey] ?? null });
      } catch (err) {
        respond(false, undefined, { message: String(err) });
      }
    },

    'goals.listSessionStrands': ({ params, respond }) => {
      try {
        const data = loadData();
        respond(true, { sessionStrandIndex: data.sessionStrandIndex });
      } catch (err) {
        respond(false, undefined, { message: String(err) });
      }
    },

    'goals.removeSessionStrand': ({ params, respond }) => {
      try {
        ...
        const data = loadData();
        delete data.sessionStrandIndex[sessionKey];
        saveData(data);
    ...
    'goals.checkConflicts': ({ params, respond }) => {
      try {
        ...
        if (!strandId) {
          respond(false, undefined, { message: 'strandId is required' });
          return;
        }
        const data = loadData();
        const strand = data.strands.find(c => c.id === strandId);
        ...
        if (!wsOps || !strand.workspace?.path) {
          respond(true, { strandId, results: [], message: 'No workspace configured' });
          return;
        }
        const goals = data.goals.filter(g => g.strandId === strandId && g.worktree?.branch);
        const results = goals.map(g => {
          const status = wsOps.checkBranchStatus(strand.workspace.path, g.worktree.branch);
          return {
            goalId: g.id,
            goalTitle: g.title,
