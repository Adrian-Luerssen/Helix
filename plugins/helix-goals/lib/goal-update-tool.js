...
export function createGoalUpdateExecutor(store) {
  ...
  return async function execute(toolCallId, params) {
    ...
    // Resolve goal: explicit goalId (strand path) or sessionIndex (single-goal path)
    let goal;
    if (goalId) {
      ...
      // If called from a strand-bound session, verify the goal belongs to the strand
      const strandId = data.sessionStrandIndex[sessionKey];
      if (strandId && goal.strandId !== strandId) {
        return error(`goal ${goalId} does not belong to the bound strand.`);
      }
    } else {
    ...
    if (ownEntry && ownEntry.goalId && ownEntry.goalId !== goal.id) {
      ...
      if (!ownGoal?.strandId || ownGoal.strandId !== goal.strandId) {
