...
/**
 * Get the PM session key for a strand
 * @param {object} store - Goals store instance
 * @param {string} strandId - Strand ID
 * @returns {string} PM session key (e.g., 'agent:main:main')
 */
export function getPmSession(store, strandId) {
  const data = typeof store.load === 'function' ? store.load() : store;
  
  // Check strand-specific PM first
  if (strandId) {
    const strand = data.strands?.find(c => c.id === strandId);
    if (strand?.pmSession) {
      return strand.pmSession;
    }
  }
...
/**
 * Get or create a dedicated PM session for a strand.
 * Creates a deterministic session key `agent:<pmAgentId>:webchat:pm-strand-<strandId>`
 * and registers it in sessionStrandIndex so the before_agent_start hook
 ...
 * @returns {{ pmSessionKey: string, created: boolean }}
 */
export function getOrCreatePmSessionForStrand(store, strandId) {
  const data = store.load();
  const strand = data.strands.find(c => c.id === strandId);
  if (!strand) {
  ...
  // Return existing PM session if it uses the correct (webchat) format
  if (strand.pmStrandSessionKey && strand.pmStrandSessionKey.includes(':webchat:pm-strand-')) {
    return { pmSessionKey: strand.pmStrandSessionKey, created: false };
  }

  // Clean up old subagent-type session key if it exists (migration)
  if (strand.pmStrandSessionKey && data.sessionStrandIndex) {
    delete data.sessionStrandIndex[strand.pmStrandSessionKey];
  }

  // Resolve PM agent ID and build deterministic key with webchat type
  const pmAgentId = getAgentForRole(data, 'pm');
  const pmSessionKey = `agent:${pmAgentId}:webchat:pm-strand-${strandId}`;

  // Store on the strand and add to sessionStrandIndex
  strand.pmStrandSessionKey = pmSessionKey;
  if (!data.sessionStrandIndex) data.sessionStrandIndex = {};
  data.sessionStrandIndex[pmSessionKey] = strandId;
  store.save(data);
