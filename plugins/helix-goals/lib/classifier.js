...
export const CLASSIFIER_CONFIG = {
  ...
  nameMatchWeight: 0.3,         // Strand name in message
 ...
 * Returns { strandId, confidence, reasoning, alternatives, tier }
 */
export function tier1Classify(message, context, strands) {
  const result = { strandId: null, confidence: 0, reasoning: null, alternatives: [], tier: 1 };

  if (!message || !strands?.length) return result;

  // 1. Explicit @strand:name mention (highest priority)
  const explicit = message.match(/@strand:(\S+)/i);
  if (explicit) {
    const target = explicit[1].toLowerCase();
    const match = strands.find(c => {
      const id = c.id.toLowerCase();
      const name = c.name.toLowerCase().replace(/\s+/g, '-');
      // Exact match on id suffix (e.g. @strand:investor-crm matches strand:investor-crm)
      // or exact match on name slug
      return id === target || id.endsWith(':' + target) || name === target;
    });
    if (match) {
      return { strandId: match.id, confidence: 1.0, reasoning: 'explicit @strand mention', alternatives: [], tier: 1 };
    }
  }
  ...
  if (context.topicId != null) {
    ...
    if (match) {
      return { strandId: match.id, confidence: 0.95, reasoning: `topic:${context.topicId}`, alternatives: [], tier: 1 };
    }
  }
  ...
  for (const strand of strands) {
    ...
    // Keyword hits
    const keywords = (strand.keywords || []).filter(k => k && k.length > 0);
    ...
    // Strand name in message
    if (strand.name && messageLower.includes(strand.name.toLowerCase())) {
      score += CLASSIFIER_CONFIG.nameMatchWeight;
      reasons.push('name');
    }

    if (score > 0) {
      scores.push({ strandId: strand.id, score: Math.min(score, 1.0), reasoning: reasons.join(' + ') });
    }
  }
  ...
  return {
    strandId: top.strandId,
    confidence,
    reasoning: top.reasoning,
    alternatives: scores.slice(1, 4).map(s => ({ strandId: s.strandId, confidence: s.score })),
    tier: 1,
  };
}
