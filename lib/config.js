 ...
 * Sharp Configuration
 * 
 * Configure Sharp by setting window.SHARP_CONFIG before loading the dashboard,
...
// Load config from window.SHARP_CONFIG or fetch from /config.json
async function loadConfig() {
  let userConfig = {};
  
  // Check for inline config
  if (window.SHARP_CONFIG) {
    userConfig = window.SHARP_CONFIG;
  } else {
...
function getConfig() {
  ...
  if (window.SHARP_CONFIG) {
    return mergeConfig(config, window.SHARP_CONFIG);
  }
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  window.SharpConfig = { getConfig, initConfig, loadConfig, DEFAULT_CONFIG };
}
