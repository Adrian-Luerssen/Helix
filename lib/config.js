 ...
 * Helix Configuration
 *
 * Configure Helix by setting window.HELIX_CONFIG before loading the dashboard,
...
function buildDefaults() {
  ...
  // Use the port the UI is served from (serve.js handles WS proxying to gateway).
  // NOTE: We no longer hardcode :18789 for localhost - the serve.js WebSocket proxy
  // at /helix-ws handles routing to the gateway AND injects goals.changed events.
  // Connecting directly to :18789 bypasses real-time goal sync.

  return {
    gatewayWsUrl: `${wsProtocol}//${hostname}${portPart}/`,
    gatewayHttpUrl: `${protocol}//${hostname}${portPart}`
  };
}
...
// Load config from window.HELIX_CONFIG or fetch from /config.json
async function loadConfig() {
  let userConfig = {};

  // Check for inline config
  if (window.HELIX_CONFIG) {
    userConfig = window.HELIX_CONFIG;
  } else {
...
function getConfig() {
  ...
  if (window.HELIX_CONFIG) {
    return mergeConfig(config, window.HELIX_CONFIG);
  }
...
const _exports = { getConfig, initConfig, loadConfig, DEFAULT_CONFIG };
if (typeof module !== 'undefined' && module.exports) {
  module.exports = _exports;
}
if (typeof window !== 'undefined') {
  window.HelixConfig = _exports;
}
